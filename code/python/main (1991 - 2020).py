...
# needed time range 1961 - 2020
time_range_start_needed = datetime.date(1991, 1, 1)
time_range_end_needed = datetime.date(2020, 12, 31)

csv_file_name = "data (1991 - 2020).csv"
...
def day_csv_data_filler(day_files_dir_list, continent_name, csv_file_dir):
    ...
    for day_file_path in day_files_dir_list:
        ...
        #getting time range
        file_time_range_start = datetime.date.fromisoformat(day_data_file[37][0])
        file_time_range_end = datetime.date.fromisoformat(day_data_file[-1][0])


        # skip all files which are not in the time range 1961 - 2020
        #ERROR FIX IT
        if file_time_range_start > time_range_start_needed or file_time_range_end < time_range_end_needed:
            #print("lines:",data_number_of_lines)
            #print("Start: ", file_time_range_start, time_range_start_needed)
            #print("End:", file_time_range_end, time_range_end_needed)
            print(f"SKIP {counter_actions} time range")
            counter_actions += 1
            continue
        ...
        if ((data_number_of_lines - counter) / data_number_of_lines) * 100 < 90:
            print("quality", counter / data_number_of_lines)
